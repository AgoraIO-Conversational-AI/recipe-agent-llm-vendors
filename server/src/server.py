# -*- coding: utf-8 -*-
"""
Agora Agent & Token Service — LLM Vendors Recipe

HTTP APIs:
- GET  /get_config     -> Generate connection config
- POST /startAgent     -> Start agent (LLM leg selected via LLM_VENDOR)
...
# FastAPI application
app = FastAPI(
    title="Agora LLM Vendors Recipe Service",
    version="1.0.0",
    description="Agora Conversational AI — swappable LLM vendor switchboard",
)
...
class StartAgentRequest(BaseModel):
    ...
    rtcUid: int
    userUid: int
    vendor: Optional[str] = None   # which LLM vendor to use (defaults to LLM_VENDOR / openai)
...
def _generate_channel_name() -> str:
    return f"llmvendors-{int(time.time())}-{random.randint(1000, 9999)}"
...
async def list_vendors():
    """List the selectable LLM vendors for the in-UI switcher."""
    import vendors as registry
    default = os.getenv("LLM_VENDOR", "openai")
    return {
        "code": 0,
        "data": {
            "default": default,
